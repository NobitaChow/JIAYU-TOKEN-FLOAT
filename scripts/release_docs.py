...
manual=a.output/'JIAYU-TOKEN-FLOAT-2.0.0-MANUAL-ZH-CN.pdf'
c=canvas.Canvas(str(manual),pagesize=(595,842));c.setTitle('JIAYU TOKEN FLOAT 2.0 中文使用说明书');c.setAuthor('JIAYU STUDIO')
...
def header(n,title,sub):
 ...
 txt(118,63,'JIAYU STUDIO',12,'Helvetica-Bold');txt(118,85,'TOKEN FLOAT  /  2.0',11,'Helvetica',muted)
...
lines(262,['打开 DMG，将应用拖入「应用程序」文件夹。','拷贝完成后，从「应用程序」打开 JIAYU TOKEN FLOAT 2.0。','最后弹出安装盘。更新已有安装时，请先退出正在运行的旧版本。'])
txt(48,358,'02  操作：拖动、展开、选择',17,'SongBold')
lines(388,['按住悬浮窗左侧四点拖动位置；再次启动会恢复保存的位置。','点击右侧箭头展开；点击窗口外收起，图钉可固定详细信息。','右下角手柄调整宽度和展开高度，位置与尺寸会自动保存。','「对话／分项目／全部项目」切换范围，设置中选择折叠指标。'])
txt(48,506,'03  定制：只留下你想看的数值',17,'SongBold')
lines(536,['在设置中选择折叠时显示的第一项和第二项，设置会自动保存。','总量旁箭头显示精确 tokens 增量，每次新增会闪烁三次。','数值上涨为绿色、下降为红色；可关闭动画并修改价格与汇率。','菜单栏图标可以重新显示悬浮窗、重置位置或退出程序。'])
...
lines(262,['tokens 总量 = 输入 + 输出。缓存输入已包含在输入中，','推理输出已包含在输出中，这两项不会再次叠加到总量。','输出均速 = 最近 60 秒日志记录的输出 tokens ÷ 60。','每分钟 tokens 为近 60 秒输入加输出，增量箭头不是每秒速度。'])
txt(48,374,'美元与人民币',17,'SongBold')
lines(404,['金额显示到 0.01。美元/分钟表示最近 60 秒已记录的等价费用。','金额是模型 tokens 的 API 等价估算，不是订阅扣费或账户账单，','不包括工具费用。缺价模型不计入金额，详情列出未计入的模型。','人民币默认使用手动参考汇率 7.0，不是实时汇率，可自行修改。'])
txt(48,517,'本地数据与常见问题',17,'SongBold')
lines(547,['只读本机可访问的会话及归档日志；未同步的远程任务无法统计。','本工具不联网、不读取钥匙串或认证文件；自动收起只观察鼠标，','不拦截其他应用输入。数值缓存不包含对话正文。','看不到窗口：点击菜单栏图标，选择重新显示或重置位置。','显示读取失败：在设置中重新选择可读取的 sessions 目录。'])
...
t(114,48,'JIAYU STUDIO · TOKEN FLOAT',20,'#ffffff','Helvetica-Bold');t(114,72,'家宇工作室 · 用量悬浮窗 2.0.0',12,'#ddede5')
...
t(48,537,'macOS 13+  /  Apple Silicon',10,'#6f7771','Helvetica');t(750,537,'2.0.0 · GUIDE',10,'#6f7771','Helvetica',True)
c.save();print(manual)
